...
public class OrderedDoubleLinkedList<T> extends DoubleLinkedList<T> implements OrderedListADT<T> {
	...
	public void add(T elem){ //O(n)-> Coste lineal
		...
		if(last == null) { //O(1)
			...
			}while(!enc && actual.prev!=last);  //n x O(1) -> O(n) donde n es el n�mero de elementos de las lista hasta encontrar el elemento
			if(enc) { //O(1)
	...
	public OrderedDoubleLinkedList<T> intersection(OrderedDoubleLinkedList<T> lista){ //O(n)-> Coste lineal
		...
		while(actual1 != this.last && actual2!=lista.last) { //n x O(1) -> O(n) donde n es el n�mero de elementos de las listas
	...
	@Override
	public lab3.OrderedDoubleLinkedList<T> intersection(lab3.OrderedDoubleLinkedList<T> lista) {

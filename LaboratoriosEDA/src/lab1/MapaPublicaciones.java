...
public class MapaPublicaciones {
	...
	public void cargarFicheroAutoresPorPublicacion(String nom) { // O(n) → Coste lineal
	    try (Scanner entrada = new Scanner(new FileReader(nom))) { // O(1)
	        while (entrada.hasNextLine()) { // n × O(1) → O(n)
	...
	public void guardarFicheroAutoresPorPublicacion(String nom) { //O(n) -> Coste lineal
		try {
			...
			for(String idP: mapaAutores.keySet()) { //n x O( m) -> O(nxm) -> O(n) donde n es el n�mero de claves del mapaAutores
				for(String idA: mapaAutores.get(idP)) { //m x O(1) -> O(m) donde m es el n�mero de elementos 
